...
namespace ClothDynamics
{
    ...
    [CustomEditor(typeof(BodySDFTexture))]
    public class BodySDFTextureEditor : Editor
    {
        enum Axis { X, Y, Z, None }

        static Mesh s_Quad;
        static BodySDFTexture s_SDFTexture;
        ...
        void OnEnable()
        {
            ...
            s_SDFTexture = target as BodySDFTexture;

            UnityEditor.SceneView.duringSceneGui -= OnSceneGUI;
            UnityEditor.SceneView.duringSceneGui += OnSceneGUI;

            m_SDF = serializedObject.FindProperty("_SDF");
            m_Size = serializedObject.FindProperty("_Size");
            m_Resolution = serializedObject.FindProperty("_Resolution");
        ...
        static void DoBounds(BodySDFTexture sdftexture)
        {
        ...
        static void DoSDFSlice(Matrix4x4 matrix, Camera camera, Vector3Int voxelResolution, Bounds voxelBounds, float distanceScale, Texture sdf)
        {
            ...
            if (s_Material == null)
                s_Material = new Material(Shader.Find("Hidden/BodySDFTexture"));
        ...
        static void OnSceneGUI(UnityEditor.SceneView sceneview)
        {
            ...
            if (s_SDFTexture.sdf != null && voxelBounds.extents != Vector3.zero && s_Axis != Axis.None)
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            BodySDFTexture sdftexture = target as BodySDFTexture;
...
#else
                case BodySDFTexture.Mode.None:
#endif
                    EditorGUILayout.HelpBox("CD will automatically create a texture at runtime, you can leave it blank!\nYou can also assign a static SDF Texture3D, or a RenderTexture here.", MessageType.Warning);
                    break;
            }

#if HAS_PACKAGE_DEMOTEAM_MESHTOSDF
            if (sdftexture.mode == SDFTexture.Mode.Static)
#else
            if (sdftexture.mode == BodySDFTexture.Mode.Static)
#endif
            {
                GUI.enabled = false;

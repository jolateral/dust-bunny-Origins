Shader "ClothDynamics/DebugUnlitShaderV2"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                if (_showDebugColors && _trisMode) {
                    float3 pAX = data.pr.xyz, pBX = data.nId.xyz, pCX = data.temp.xyz;
                    float3 normalX = normalize(cross(pBX - pAX, pCX - pAX));
                    o.vertex.xyz += normalX * _normalScale * data.mask * 0.01; // * nextId TODO? see CollisionMeshesGPU
                }
                ...
                o.color = _showDebugColors ? data.color : 1;

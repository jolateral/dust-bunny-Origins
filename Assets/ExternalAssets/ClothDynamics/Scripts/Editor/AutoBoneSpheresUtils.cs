...
{
    using BoneShape = AutomaticBoneSpheres.BoneShape;

    [InitializeOnLoad]
    public static class AutoBoneSpheresUtils
    {
        ...
        static List<AutomaticBoneSpheres> s_BoneRendererComponents = new List<AutomaticBoneSpheres>();
        ...
        static AutoBoneSpheresUtils()
        {
            AutomaticBoneSpheres._onAddBoneRenderer += OnAddBoneRenderer;
            AutomaticBoneSpheres._onRemoveBoneRenderer += OnRemoveBoneRenderer;
        ...
        static void DrawSkeletons(SceneView sceneview)
        {
            ...
            for (var i = 0; i < s_BoneRendererComponents.Count; i++)
            {
                ...
                if (boneRenderer._drawBones)
                {
                    var size = boneRenderer._boneSize * 0.025f;
                    var shape = boneRenderer._boneShape;
                    var color = boneRenderer._boneColor;
                    var nubColor = new Color(color.r, color.g, color.b, color.a);
                    var selectionColor = Color.white;

                    for (var j = 0; j < boneRenderer._bones.Length; j++)
                    {
                        var bone = boneRenderer._bones[j];
                        if (bone.first == null || bone.second == null)
                            continue;

                        DoBoneRender(bone.first, bone.second, shape, color, size);
                    }
                    if (boneRenderer._tips != null)
                        for (var k = 0; k < boneRenderer._tips.Length; k++)
                        {
                            var tip = boneRenderer._tips[k];
                            if (tip == null)
                                continue;
                ...
                if (boneRenderer._drawTripods)
                {
                    var size = boneRenderer._tripodSize * 0.025f;
                ...
                case EventType.MouseDown:
                    {
                        if (HandleUtility.nearestControl == id && evt.button == 0)
                        {
                            if (!SceneVisibilityManager.instance.IsPickingDisabled(boneGO, false))
                            {
                                ...
                                EditorHelper.HandleClickSelection(boneGO, evt, s_BoneRendererComponents);
                                evt.Use();
                            }
                        }
                        break;
        ...
        public static void OnAddBoneRenderer(AutomaticBoneSpheres obj)
        {
            if(!s_BoneRendererComponents.Contains(obj)) s_BoneRendererComponents.Add(obj);
        }

        public static void OnRemoveBoneRenderer(AutomaticBoneSpheres obj)
        {
    ...
    public static class EditorHelper
    {
        ...
        public static void HandleClickSelection(GameObject gameObject, Event evt, List<AutomaticBoneSpheres> boneRendererComponents)
        {
            ...
            int length = boneRendererComponents.Count;
            for (int i = 0; i < length; i++)
            {
                var boneScript = boneRendererComponents[i];
                var bones = boneScript._bones.Select(p => p.first).ToArray();
                boneScript._selectedBones.Clear();
                var selection = Selection.objects.Where(p => bones.Contains(((GameObject)p).transform));
                foreach (var item in selection)
                {
                    boneScript._selectedBones.Add(System.Array.IndexOf(bones, ((GameObject)item).transform));
                }
            }
        }

        static public bool DrawHeader(string text, float width = float.NaN)
        {
            return DrawHeader(text, text, false, new Color(0.5f, 0.8f, 1.0f), new Color(0.3f, 0.7f, 1.0f), Color.white, width);
        }

        static public bool DrawHeader(string text, string key, float width = float.NaN)
        {
            return DrawHeader(text, key, false, new Color(0.5f, 0.8f, 1.0f), new Color(0.3f, 0.7f, 1.0f), Color.white, width);
        }

        static public bool DrawHeader(string text, bool forceOn, float width = float.NaN)
        {
            return DrawHeader(text, text, forceOn, new Color(0.5f, 0.8f, 1.0f), new Color(0.3f, 0.7f, 1.0f), Color.white, width);
        }
        static public bool DrawHeader2(string text, bool forceOn, float width = float.NaN)
        {
            return DrawHeader(text, text, forceOn, new Color(1.0f, 0.7f, 0.7f), new Color(1.0f, 0.5f, 0.5f), new Color(0.8f, 0.8f, 0.8f), width);
        }
        static public bool DrawHeader(string text, string key, bool forceOn, Color b1, Color b2, Color bg, float width = float.NaN)
        {
            bool state = EditorPrefs.GetBool(key, true);

            EditorGUILayout.Space();
            if (!forceOn && !state)
                GUI.backgroundColor = b1;
            else
                GUI.backgroundColor = b2;
#if UNITY_3_5
					if (!GUILayout.Toggle(true, text, "dragtab")) state = !state;
#else
            if (!GUILayout.Toggle(true, "<b><size=12>" + text + "</size></b>", "dragtab", GUILayout.Height(13)))
                state = !state;
#endif

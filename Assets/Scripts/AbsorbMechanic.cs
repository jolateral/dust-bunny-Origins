...
public class AbsorbMechanic : MonoBehaviour
{
    ...
    [Tooltip("How much extra the bunny swells up during absorption before settling down. (e.g., 0.15 creates a very noticeable 'gulp' expansion)")]
    public float swellAmount = 0.15f; // NEW: Explicit punch size for the model
    ...
    void AttemptAbsorb(GameObject item)
    {
        ...
        orbit.extraHeightOffset = orbitHeightOffset;
    ...
    public void SpillAbsorbables(int requestedCount)
    {
        ...
        // Sync the currentTargetScale so future growth starts from the correct spilled size
    ...
    // =======================================================================
    // NEW: SINE WAVE GROWTH COROUTINE (Massive Gulp Effect purely on the model)
    // =======================================================================
    ...
    /// <summary>
    /// Coroutine that animates the model expanding significantly using a Sine Wave 
    /// before shrinking back to its true new size. This ensures the visual feedback 
    /// is obvious even if the actual growth factor is tiny.
    /// </summary>
    private IEnumerator SmoothScaleRoutine()
    {
        ...
        while (elapsed < smoothGrowthDuration)
        {
            ...
            float t = Mathf.Clamp01(elapsed / smoothGrowthDuration);

            // 1. Calculate the base interpolation (slowly reaching the real target size)
            Vector3 baseScale = Vector3.Lerp(startScale, currentTargetScale, t);

            // 2. Add an absolute "swelling" effect using a Sine Wave. 
            // Mathf.Sin goes 0 -> 1 -> 0 over the duration, causing it to bloat in the middle.
            float currentSwell = Mathf.Sin(t * Mathf.PI) * swellAmount;

            transform.localScale = baseScale + (Vector3.one * currentSwell);
...
public class OrbitBehaviour : MonoBehaviour
{
    ...
    public float absorbDuration = 0.4f; 
